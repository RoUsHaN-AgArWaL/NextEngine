...
async function afterPackageBuilt (path, arch) {
  var installerOptions = {
    artifactName: 'nextengine-${version}-${arch}.deb',
    ...
    description: 'NextEngine is a fast, minimal browser that protects your privacy.',
    synopsis:
      'A fast, minimal browser with smarter search, improved tab management, and built-in ad blocking.',
    depends: [
      'libsecret-1-0',
      'libasound2',
      'libc6',
...
;(async () => {
  for (const arch of arches) {
    await createPackage('linux', { arch })
      .then(path => afterPackageBuilt(path, arch))
  }
})()
