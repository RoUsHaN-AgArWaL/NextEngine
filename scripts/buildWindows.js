...
async function afterPackageBuilt (packagePath) {
  ...
  const output = fs.createWriteStream(
    'dist/app/NextEngine-v' + version + '-windows' + archSuffix + '.zip'
  )

  const archive = archiver('zip', { zlib: { level: 9 } })
  archive.directory(packagePath, 'NextEngine-v' + version)
  archive.pipe(output)
  await archive.finalize()
  ...
  const options = {
    src: packagePath,
    dest: 'dist/app/nextengine-installer' + archSuffix,
    icon: 'icons/icon256.ico',
    animation: 'icons/windows-installer.gif',
    licenseUrl: 'https://github.com/yourrepo/nextengine/blob/master/LICENSE.txt',
    noMsi: true
  }

  console.log('Creating NextEngine installer...')
  ...
  await installer(options)
    .then(() => {
      fs.renameSync(
        './dist/app/nextengine-installer' + archSuffix + '/NextEngine-' + version + '-setup.exe',
        './dist/app/NextEngine-' + version + archSuffix + '-setup.exe'
      )
    })
    .catch(err => {
      console.error(err)
