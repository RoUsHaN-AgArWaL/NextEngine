...
if (electron.contextBridge) {
  electron.contextBridge.exposeInMainWorld('downloadManagerAPI', {
    list: () => ipc.invoke('download-manager-list'),
    remove: (id) => ipc.invoke('download-manager-remove', id),
    clear: () => ipc.invoke('download-manager-clear'),
    openFile: (path) => ipc.invoke('download-manager-open-file', path),
    openFolder: (path) => ipc.invoke('download-manager-open-folder', path),
    onUpdate: (callback) => ipc.on('download-manager-update', (event, info) => callback(info))
  })
}
