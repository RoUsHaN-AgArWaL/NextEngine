...
const webviews = {
  ...
  if (webviews.viewFullscreenMap[webviews.selectedId]) {
    ...
    // ✅ ADD THIS (your second navbar height)
    const secondaryNavbarHeight = 50  // 🔥 change this value as needed

    const totalTop = navbarHeight + secondaryNavbarHeight

    const viewMargins = webviews.viewMargins

    let position = {
      x: Math.round(viewMargins[3]),
      y: Math.round(viewMargins[0]) + totalTop,
      width: window.innerWidth - Math.round(viewMargins[1] + viewMargins[3]),
      height: window.innerHeight - Math.round(viewMargins[0] + viewMargins[2]) - totalTop
    }
